...
struct PipelineHistoryStoreRecoveryTests {
    static func main() throws {
        ...
        try testExplicitInMemoryStoreRejectsDurableUpdatesAndDeletions()
        try testUpdateAndDeleteRejectMissingHistoryEntries()
    ...
    private static func testExplicitInMemoryStoreRejectsDurableUpdatesAndDeletions() throws {
        let store = PipelineHistoryStore(inMemory: true)
        let item = PipelineHistoryItem(
            id: UUID(),
            timestamp: Date(timeIntervalSince1970: 1_000),
        ...
        try expectDurableStoreUnavailable {
            try store.update(item, requiresDurableStore: true)
        }
        try expectDurableStoreUnavailable {
            _ = try store.delete(id: item.id, requiresDurableStore: true)
        }
        try expectDurableStoreUnavailable {
            _ = try store.clearAll(requiresDurableStore: true)
        }
    }

    private static func testUpdateAndDeleteRejectMissingHistoryEntries() throws {
        let store = PipelineHistoryStore(inMemory: true)
        let id = UUID()
        let item = PipelineHistoryItem(
            id: id,
            timestamp: Date(timeIntervalSince1970: 1_000),
            rawTranscript: "Missing history item.",
            postProcessedTranscript: "Missing history item.",
            postProcessingPrompt: nil,
            contextSummary: "",
            contextScreenshotDataURL: nil,
            contextScreenshotStatus: "No screenshot",
    ...
    private static func expectMissingHistoryEntry(
        _ operation: () throws -> Void
    ) throws {
        do {
            try operation()
            throw RecoveryTestFailure("Missing history entries must reject mutation")
        } catch PipelineHistoryStoreError.historyEntryNotFound {
            // expected
        }
    }

    private static func expectDurableStoreUnavailable(
        _ operation: () throws -> Void
    ) throws {
        do {
            try operation()
            throw RecoveryTestFailure("In-memory stores must reject durable operations")
        } catch PipelineHistoryStoreError.durableStoreUnavailable {
            // expected
        }
    }
